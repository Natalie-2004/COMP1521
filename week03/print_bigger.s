...
scan_loop__body:
	...
	la	$t2, numbers		
	add	$t2, $t2, $t1		
	sw	$v0, ($t2)			#   scanf("%d", &numbers[i]); -> Store input in $v0 to the memory address pointed to by $t2: numbers[i].

	move $t3, $v0			# final_number = number[i] -> copy $t0 to $v3
...
print_loop__body:
	...
	add	$t2, $t2, $t1		
	lw	$a0, ($t2)			#
	blt 	$a0, $t3, print_loop_stepper	#   if (numbers[i] >= final_number)
