...
void perform_increment(void) {
    pthread_mutex_lock(&counter_mutex);   // Lock the mutex before modifying the shared resource
    global_counter = global_counter + 1;
    pthread_mutex_unlock(&counter_mutex); // Unlock the mutex after modifying the shared resource
}
